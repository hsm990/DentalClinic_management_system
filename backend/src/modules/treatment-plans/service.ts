...
async function createPlan(user: RequestingUser, patientId: string, data: any) {
  await verifyPatientAccess(user, patientId);
  return treatmentPlansRepository.createPlan(patientId, data);
}
...
async function addItem(user: RequestingUser, planId: string, data: any) {
  ...
  // procedureId is client-supplied — confirm it belongs to this clinic,
  // same rule as the catalog and appointments modules
  ...
  return treatmentPlansRepository.addItem(planId, data);
}
...
export default { listPlans, createPlan, addItem, updateItemStatus };
